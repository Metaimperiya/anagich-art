...
/* Языки, которые появятся на следующих этапах */
const lazyLoaders = {
  de: () => import("./de.js"),
  da: () => import("./da.js"),
  fi: () => import("./fi.js"),
...
/**
 * Установить язык. Динамически подгрузит словарь, если его нет.
 */
...
/**
 * Получить перевод по ключу. Поддерживает вложенность через точку.
 * Если ключа нет — вернёт сам ключ (чтобы не ломать верстку).
 */
...
/**
 * Проходит по DOM и подставляет переводы в элементы с data-i18n.
 * Также поддерживает data-i18n-placeholder для placeholder-атрибутов.
 */
...
/**
 * Список доступных языков (для dropdown).
 */
